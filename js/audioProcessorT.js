...
const createSilence = (duration) => {
  return new Promise((resolve, reject) => {
    ...
    ffmpeg()
      ...
      .on("error", (err) =>
        reject(new Error(`Error creating silence: ${err.message}`))
      )
      .run();
  });
};
 ...
 */
const concatenateMP3Files = (inputFiles, outputFile) => {
  return new Promise((resolve, reject) => {
    ...
    command
      .on("error", (err) =>
        reject(new Error(`Error concatenating files: ${err.message}`))
      )
      .on("end", () => resolve())
 ...
 */
const copyAndRenameFile = async (source, destination) => {
  try {
...
const cleanUpSilenceFile = async () => {
  try {
    ...
    await Promise.all(
      silenceFiles.map((file) => fs.promises.unlink(path.join(destDir, file)))
    );
    console.log("Silence files cleaned up");
  } catch (err) {
    throw new Error(`Error cleaning up silence files: ${err.message}`);
  }
};
 ...
 */
const generateAudioFolder = async (writer, grade, group, errorMessages) => {
  /**
  ...
  try {
        ...
        Object.entries(audioArr).forEach((audioSource) => {
          ...
          if (audioType === "q") {
            ...
            if (audioFileList.length === 4) {
              ...
              audioFileList.map((file, idx) => {
                ...
                if (abs === "A") {
                  ...
                  errorMessages.push(`${id} media ${file} ABC Type conflict`);
                }
              });
              ...
              errorMessages.push(`${id} media ${audioType} length over 4`);
            }
          } else if (audioType === "c") {
            let choiceList = {};
            let mergeBaseFile = "";
            ...
            audioFileList.forEach((file) => {
              ...
              if (split.length !== 4) {
                errorMessages.push(`Error: media name is strange: ${file}`);
              } else {
                ...
                if (choiceNum === "01" && article === "A") {
                  if (mergeBaseFile) {
                    errorMessages.push(`Error: already exist file: ${file}`);
                  } else {
                    mergeBaseFile = file;
                  }
                }
            ...
            Object.entries(choiceList).forEach((data, i) => {
              ...
              files.forEach((file) => {
                ...
                if (abc === "A") {
                  if (Afile) {
                    errorMessages.push(`Error: already exist Afile: ${file}`);
                  } else {
                    Afile = file;
                  }
                } else if (abc === "B") {
                  if (Bfile) {
                    errorMessages.push(`Error: already exist Bfile: ${file}`);
                  } else {
                    Bfile = file;
                  }
                } else if (abc === "C") {
                  if (Cfile) {
                    errorMessages.push(`Error: already exist Cfile: ${file}`);
                  } else {
                    Cfile = file;
                  }
                } else if (abc === "D") {
                  if (Dfile) {
                    errorMessages.push(`Error: already exist Dfile: ${file}`);
                  } else {
                    Dfile = file;
                  }
                } else {
                  errorMessages.push(`media ${file} is not ABCD Type conflict`);
                }
              });
              ...
              if (Afile && Bfile && Cfile && Dfile) {
                ...
                if (!mergeBaseFile) {
                  errorMessages.push(`Error: No Base file: ${id}`);
                  return;
                }
                ...
                errorMessages.push(`Error: Strange fileList : ${files}`);
                return;
              }
            });
          } else if (audioType === "a") {
            ...
            if (audioFileList.length > 2) {
              errorMessages.push(`${id} Answer media ${audioFileList} over 2`);
            } else if (audioFileList.length === 2) {
              mediaPaths.push(mediaSavePath);
              tasks.push(
                new Promise((resolve, reject) => {
                  createSilence(duration, (err, silenceFilePath) => {
                    if (err) {
                      errorMessages.push("Failed to create silence file:", err);
                      resolve();
                      return;
                    }
    ...
    return { success: true, message: "All files processed successfully." };
  } catch (err) {
    // throw err;
    errorMessages.push(`Critical Error: ${err.message}`);
    return { success: false, errors: errorMessages };
  }
};
 ...
 * @param {3 | 4} grade 학년
 */
const importExel = async (writer, grade, errorMessages) => {
  ...
  fileList.map((fileName) => {
    ...
    if (split.length === 4) {
      ...
      errorMessages.push(`Error Data: ${fileName}`);
    }
  });
  ...
  Object.entries(group?.[writer]?.[grade]).map((data) => {
    ...
    Object.entries(chpData).map((chpD) => {
      ...
      if (id.includes("S") || id.includes("A2") || id.includes("P3")) {
        if (!keys.includes("a")) errorMessages.push(`${id}: no Answer MP3`);
      }
    });
  });

  try {
    await generateAudioFolder(writer, grade, group, errorMessages);
    ...
    errorMessages.push(`Error generating audio folder: ${err.message}`);
  }
};
...
const processAudio = async (newBasePath, newDestPath, author, grade) => {
  basePath = path.join(newBasePath, "resource"); // 전역 변수 basePath 설정
  inputDir = newDestPath; // 전역 변수 inputDir 설정
  destDir = newDestPath; // 전역 변수 destDir 설정
  const errorMessages = []; // 에러 메시지를 수집할 배열

  try {
    await importExel(author, grade, errorMessages); // 에러 메시지 배열을 전달
  } catch (err) {
    errorMessages.push(`Error processing audio: ${err.message}`);
  }
